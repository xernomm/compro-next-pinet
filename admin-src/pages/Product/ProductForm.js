...
const ProductForm = () => {
    ...
    const fetchProduct = async () => {
        try {
            ...
            if (product.video_url && (product.video_url.startsWith('/') || product.video_url.includes('/uploads/'))) {
                setPreviewVideo(getImageUrl(product.video_url));
            }
        } catch (error) {
            ...
            <div className="content-card form-page-content">
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="form-section">
                        ...
                        <div className="form-group">
                            <label>Or Upload Video File (mp4, mov, mkv, avi, webm)</label>
                            <input type="file" accept="video/mp4,video/quicktime,video/x-matroska,video/x-msvideo,video/webm" onChange={handleVideoChange} className="form-control" />
                            {previewVideo && (
                                <div style={{ marginTop: '10px' }}>
                                    <video src={previewVideo} controls style={{ maxHeight: '180px', maxWidth: '100%', borderRadius: '4px', background: '#000' }} />
                                    <button 
                                        type="button" 
                                        className="btn btn-sm btn-danger d-block mt-1" 
                                        onClick={() => {
                                            setVideo(null);
